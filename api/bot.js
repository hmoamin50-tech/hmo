...
async function sendMessageToGemini(userId, text) {
    ...
    // تنظيف المحادثات القديمة (أكثر من 30 دقيقة)
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (data.error && data.error.message.includes('API key')) {
                throw new Error('مشكلة في مفتاح API - تأكد من صحة المفتاح');
            }
        ...
        const botResponse = data.candidates?.[0]?.content?.parts?.[0]?.text
            ...
            : 'عذراً، لم أتلقَ إجابة واضحة من Gemini.';
        ...
        // تقليل عدد الرسائل إذا تجاوزت الحد (الحفاظ على السياق)
        ...
        if (error.message.includes('API key') || error.message.includes('403')) {
            userFriendlyError = '⚠️ مشكلة في مصادقة API. يرجى التحقق من المفتاح.';
        } else if (error.message.includes('network') || error.message.includes('fetch')) {
            userFriendlyError = '⚠️ مشكلة في الاتصال بالإنترنت. حاول مرة أخرى.';
        } else {
            userFriendlyError = `⚠️ حدث خطأ: ${error.message}`;
        }
...
bot.on('message', async (msg) => {
    ...
    if (!msg.text) {
        await bot.sendMessage(chatId, 'أستطيع فهم النصوص فقط حالياً. أرسل لي نصاً!');
        return;
    }
    ...
    try {
        ...
        const response = await sendMessageToGemini(userId, msg.text);
        ...
        let errorMessage;
        if (error.message.includes('مشكلة في مصادقة API')) {
            errorMessage = '🔑 **خطأ في المصادقة:**\n'
                         + 'يرجى التحقق من:\n'
                         + '1. صحة مفتاح Gemini API في ملف .env\n'
                         + '2. تفعيل الـ API في Google AI Studio\n'
                         + '3. التأكد من أن المفتاح غير منتهي الصلاحية';
        } else {
            errorMessage = `❌ حدث خطأ غير متوقع:\n${error.message}\n\n`
                         + 'يرجى المحاولة مرة أخرى أو استخدام /clear لمسح الذاكرة.';
        }
